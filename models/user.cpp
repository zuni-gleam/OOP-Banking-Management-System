...
{
    this->id = id; this->uname = uname; this->pass = pass; this->role = role;
    this->fullname = name; this->phone = phone; this->addr = addr;
    this->sq = sq; this->sa = sa; this->secret = secret; this->tier = tier;
}
...
void user::setphone(const QString& p) { phone = p; }
void user::setaddr(const QString& a) { addr = a; }
void user::setname(const QString& n) { fullname = n; }
void user::setpass(const QString& p) { pass = p; }
void user::setsecret(const QString& s) { secret = s; }
void user::settier(const QString& t) { tier = t; }
...
user user::loadfromid(int uid)
{
    QSqlQuery q;
    q.prepare("SELECT id, username, password, role, fullname, phone, address, "
              "securityq, securitya, totpsecret, tier FROM users WHERE id = ?");
    q.addBindValue(uid);
    q.exec();
    if (q.next())
        return user(q.value(0).toInt(), q.value(1).toString(), q.value(2).toString(),
...
bool user::savetodb() const
{
    ...
    q.addBindValue(uname); q.addBindValue(pass); q.addBindValue(role);
    q.addBindValue(fullname); q.addBindValue(phone); q.addBindValue(addr);
    q.addBindValue(sq); q.addBindValue(sa); q.addBindValue(secret); q.addBindValue(tier);
    return q.exec();
}
...
bool user::updateprofile() const
{
    ...
    q.addBindValue(fullname); q.addBindValue(phone); q.addBindValue(addr); q.addBindValue(id);
    return q.exec();
}
...
bool user::updatepass() const
{
    ...
    q.addBindValue(pass); q.addBindValue(id);
    return q.exec();
}


bool user::deleteaccount() const
{
    QSqlQuery q;
